...
def test_ui_load_uses_operation_date_for_all_three_requests(monkeypatch, tmp_path):
    ...
    assert "q=`?operation_date=${day}&current_only=true`" in html
    assert "api('cards/summary'+q,{sessionNotice:false}),api('cards'+q,{sessionNotice:false}),api('cards/missing'+missingQ,{sessionNotice:false})" in html
