...
def _observation(created, key, price, *, volume=1.5, bench=0, sector=0, invalidation=False, seconds=1):
    ...
    return {"provider": "test", "source": "test", "symbol": "123456", "idempotency_key": key, "known_at": known, "retrieved_at": known, "price_krw": price, "best_bid": price - .1, "best_ask": price + .1, "spread_pct": .3, "imbalance": 0, "volume_ratio": volume, "benchmark_excess_pct": bench, "sector_excess_pct": sector, "trusted_business_invalidation": invalidation}
...
def test_lineage_and_observation_invariants_and_all_actions(monkeypatch, tmp_path):
    ...
    endpoint = f"/api/internal/scenario-sets/{created['id']}/observations"
    ...
    for sequence, (key, price, volume, bench, sector, invalidation, expected) in enumerate(cases, 1):
        response = client.post(endpoint, json=_observation(created, key, price, volume=volume, bench=bench, sector=sector, invalidation=invalidation, seconds=sequence), headers=headers)
        assert response.status_code == 200, response.text
        assert response.json()["action"] == expected
    duplicate = _observation(created, "add", sum(ranges["GOOD"].values()) / 2, seconds=20)
    assert client.post(endpoint, json=duplicate, headers=headers).json()["idempotent"] is True
