...
def test_conditional_immutable_retry_successor_observation_refusal_and_no_highlight(monkeypatch, tmp_path):
    ...
    mismatch = deepcopy(payload); mismatch["scenarios"][0]["conditions"].append("새 문장")
...
def test_card_save_orchestrates_only_complete_judgment_hold_candidate(monkeypatch, tmp_path):
    ...
    assert db.execute("SELECT scenario_kind,expected_value_krw FROM event_scenario_sets WHERE card_id=?", (saved["id"],)).fetchone()["scenario_kind"] == "CONDITIONAL"
    assert db.execute("SELECT COUNT(*) FROM order_plans").fetchone()[0] == 0
    db.close()
...
def test_backfill_is_dry_by_default_apply_is_idempotent_and_skips_invalidated(monkeypatch, tmp_path):
    ...
    assert db.execute("SELECT COUNT(*) FROM event_scenario_sets").fetchone()[0] == 0
    applied = run(db, apply=True)
    assert applied["created"] == 1 and db.execute("SELECT COUNT(*) FROM event_scenario_sets WHERE card_id=?", (invalidated["id"],)).fetchone()[0] == 0
