...
def _calendar() -> dict[int, frozenset[date]]:
    try:
        ...
        for year_text, entries in years.items():
            ...
            if any(item.year != int(year_text) for item in parsed) or len(parsed) != len(set(parsed)):
                raise CalendarError("invalid or duplicate KRX calendar date")
            result[int(year_text)] = frozenset(parsed)
        return result
    except (OSError, TypeError, ValueError, json.JSONDecodeError) as exc:
        if isinstance(exc, CalendarError): raise
...
def previous_krx_business_date(day: date) -> date:
    ...
    while not is_krx_business_date(candidate): candidate -= timedelta(days=1)
    return candidate
...
def admitted_backlog_dates(run_date: date) -> list[str]:
    ...
    while cursor <= run_date:
        values.append(cursor.strftime("%Y%m%d")); cursor += timedelta(days=1)
    return values
