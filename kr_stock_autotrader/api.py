...
_PREMARKET_READINESS = frozenset({
    "canary_ready", "canary_recovered", "persistent_failure", "premarket_deadline", "invalid_as_of",
})
_PREMARKET_STAGES = frozenset({"research_dependency", "market_snapshot", "filter", "card", "recovery"})
...
def _enqueue_backlog(db, *, kind: str, identity: str, payload: dict, run_key: str, announcement_at: str | None = None) -> None:
    """First observation wins; unresolved work is never aged out or overwritten."""
    db.execute("INSERT OR IGNORE INTO giraffe_research_backlog(identity,kind,payload,original_announcement_at,first_run_key,created_at) VALUES(?,?,?,?,?,?)", (
        _backlog_identity(kind, identity), kind, json.dumps(payload, sort_keys=True), announcement_at, run_key,
...
def _seed_unfinished_research_backlog(db) -> None:
    ...
    for row in db.execute(
        ...
        for source in sources:
            if isinstance(source, dict) and isinstance(source.get('rcp_no'), str) and re.fullmatch(r'\d{14}', source['rcp_no']):
...
def _research_commitment(run_key: str, contract: object) -> dict:
    ...
    if contract.get("schema_version") in {"giraffe-research-control-v2", "giraffe-research-control-v3"}:
        ...
        try:
            ...
            expected = contract.get("expected_rcp_nos"); sources = contract.get("sources")
...
def _valid_control_economic_audit(source: dict, item: dict) -> bool:
    ...
    if is_correction_single_sale_supply_contract(source.get('report_name')):
        ...
        # A timing hold is valid only for recomputed qualifying facts.  A
        # zero/negative or sub-threshold amendment cannot evade its audit.
        return ((disposition == 'timing_unresolved' and qualifies)
                or (disposition != 'timing_unresolved' and (disposition == 'qualifying_A_or_better') == qualifies))
    ...
    qualifies = _qualifies_control_contract_from_validated_facts(source, facts)
    # Date-only rcept_dt is never a substitute for a publication instant.  A
    # hold is nevertheless limited to contracts that recompute as qualifying.
    return ((disposition == 'timing_unresolved' and qualifies)
...
def _terminalize_v2_backlog(db, run_key: str, commitment: dict, detail: dict) -> None:
    ...
    for item, expected_item in zip(discovery, discovery_expected):
        ...
        if row is None or dict(row) != {'status':'terminal','terminal_disposition':item['disposition'],'terminal_run_key':run_key,'terminal_evidence_id':item['evidence_id']}:
            raise HTTPException(422, 'research discovery terminal backlog readback failed')
...
def _safe_premarket_scheduler_finish(data: object) -> dict:
    """Closed projection for the 08:00 persisted diagnostic boundary.

    The card runner may receive provider failures, but this one persistence path
    stores only finite operational vocabulary.  Other scheduler kinds retain
    their existing compatibility payloads.
    """
    if not isinstance(data, dict) or data.get("status") not in {"done", "error"}:
        raise HTTPException(422, "invalid premarket scheduler finish")
    count = _nonnegative_int(data.get("count", 0))
    detail = data.get("detail", {})
    if count is None or not isinstance(detail, dict):
        raise HTTPException(422, "invalid premarket scheduler detail")
    safe: dict[str, object] = {}
    for name in ("attempt_count", "eligible_count", "completed_count"):
        if name in detail:
            value = _nonnegative_int(detail[name])
            if value is None:
                raise HTTPException(422, "invalid premarket scheduler detail")
            safe[name] = value
    if "readiness" in detail:
        if detail["readiness"] not in _PREMARKET_READINESS:
            raise HTTPException(422, "invalid premarket scheduler detail")
        safe["readiness"] = detail["readiness"]
    if "stage" in detail:
        if detail["stage"] not in _PREMARKET_STAGES:
            raise HTTPException(422, "invalid premarket scheduler detail")
        safe["stage"] = detail["stage"]
    if "failure_reasons" in detail:
        failures = detail["failure_reasons"]
...
@app.get('/api/internal/research-backlog')
def research_backlog(_: None = Depends(require_research_control_key)):
    """Prehook-only durable pending cursor; terminal history is never silently purged."""
    db = connect()
    try:
