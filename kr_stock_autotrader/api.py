...
HTML = '''<!doctype html><html lang="ko"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Giraffe · 국내주식 모의투자</title>
...
<h1>Giraffe</h1><p>국내주식 자동매매 계획</p><p class=warn><b>모의투자 전용</b> · 실주문은 영구 비활성화 · 비용/세금/슬리피지 0</p><section><input id=e placeholder=이메일><input id=p type=password placeholder="비밀번호 8자 이상"><button onclick="auth('signup')">가입</button><button onclick="auth('login')">로그인</button><button onclick="logout()">로그아웃</button></section><section><h2>매수 계획</h2><div class=row><input id=s value=005930 aria-label="계획 종목코드"><input id=n value=삼성전자><input id=d type=datetime-local><input id=q type=number value=1 min=1><select id=o><option value=market>시장가</option><option value=limit>지정가</option></select><input id=l type=number placeholder=지정가></div><select id=combine><option>OR</option><option>AND</option></select><div id=conds></div><button onclick=addCond()>매도 조건 추가</button><small>조건을 비워두면 자동매도하지 않습니다.</small><button onclick=save()>계획 저장</button></section><section><h2>시세 입력/평가</h2><div class=row><label for=tickSymbol>평가할 종목코드 (6자리)<input id=tickSymbol inputmode=numeric pattern="[0-9]{6}" maxlength=6 value=005930></label><input id=price type=number value=70000 aria-label=현재가><input id=vol type=number value=100 aria-label=거래량><input id=base type=number value=100 aria-label=기준거래량><input id=known type=datetime-local aria-label=알려진시각></div><button onclick=tick()>모의 tick 평가</button></section><section><button onclick=list()>내 계획 새로고침</button><p id=message aria-live=polite>로그인하세요.</p><div id=plans></div><details><summary>디버그 JSON</summary><pre id=out></pre></details></section>
