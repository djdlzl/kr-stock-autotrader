...
- manual noon recovery는 `research_mode=manual_catch_up`, `eligible_for_original_cutoff=false`로만 evidence를 저장한다. `announcement_at`은 원문 발표시각, `known_at`은 이번 수동 확인시각이며 API가 서버 관측 `collected_at`을 기록한다. 제출한 collection 시각은 timezone-aware이고 `announcement_at <= known_at <= submitted_collected_at <= server_collected_at`일 때만 허용된다. 이는 delayed/current evidence이고 과거 07:00 performance/as-of input이 아니다. `research_mode`를 생략해도 scheduled_as_of이며 그 run-date의 `known_at <= 07:00 KST` cutoff을 우회할 수 없다.
