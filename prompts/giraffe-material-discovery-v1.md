...
- 각 lane은 `checked_url_count=len(checked_sources)`, `source_valid_count=sum(source_valid=true)`, `candidate_count=sum(outcome=candidate)`, `query_count=len(queries)`를 정확히 맞춘다. `kind_krx`만 `executed=true` 및 query/source-valid 각각 최소 1개가 필요하다. issuer/newsroom과 media는 optional이며 unattempted면 모든 count와 list가 0/empty여야 한다; 시도한 URL failure는 `coverage_error_count`와 top-level `coverage_error`에 남기되 DONE을 막지 않는다. completion API가 이 bounded inline receipt를 scheduler DB finish detail에 그대로 영속한다.
...
- terminal disposition, category totals, packet/provenance, 그리고 성공 evidence readback이 모두 정확하면 source_error/store_error 및 optional-lane coverage_error가 있어도 `scheduler-finish ... done`으로 partial completion을 저장한다.
- DART가 빈 manifest(`control_count=0`)여도 KIND lane의 실제 완료 전에는 `done`, count=0이 불가하다. KIND가 source-valid 원문 확인으로 후보 0건을 결론내면 count=0 done이 가능하며, issuer/media는 optional이다. source-valid 후보가 하나라도 있으면 그 후보별 semantic terminal result 없이 done을 호출하지 않는다.
