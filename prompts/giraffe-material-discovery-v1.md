...
- v3 control terminal receipt는 exact `{rcp_no,disposition,evidence_id,economic_disposition,economic_reason,economic_facts}` audit를 남긴다. 개별 원문/분류 실패는 `disposition=source_error`, 개별 저장/readback 실패는 `disposition=store_error`, 두 경우 모두 `evidence_id=null`, `economic_disposition=error`, 비어 있지 않은 bounded `economic_reason`, `economic_facts=null`로 닫고 다음 receipt를 계속 처리한다. 새 `단일판매ㆍ공급계약체결`은 full-contract facts `{binding_contract,contract_amount,prior_revenue,ratio_percent,term}`를 원문에서 추출하고 binding contract이며 `contract_amount / prior_revenue >= 50%`이면 `rejected|hold`은 API에서 거부되며 evidence-bound `saved|correction_stored`만 가능하다. 정확히 `[기재정정]`로 시작하는 표준 공급계약 정정 title은 legacy total을 쓰지 말고 closed amendment facts `{binding_contract,economic_basis:"amendment_delta",original_contract_amount,amended_contract_amount,incremental_contract_amount,prior_revenue,incremental_ratio_percent,original_term,amended_term}`를 쓴다. 금액은 bool이 아닌 정수이고 increment는 `amended-original`과 정확히 같아야 하며, 등급은 total이 아닌 양의 increment / prior revenue로 판정한다. original/amended term을 모두 원문 그대로 보존한다. 양의 increment가 50% 이상인 binding correction만 A 이상으로 처리한다. 다른 filing class에는 추정 facts를 만들지 말고 `economic_facts=null`로 남긴다. 이는 title-bound closed-shape dispatch이며 v3 schema version은 새 계약 legacy facts와의 호환성을 유지한다.
