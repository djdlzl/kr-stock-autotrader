...
- 각 complete manifest의 `material_candidate_records`가 DART 조사 제어 목록이다. `source_packet_paths`는 source-valid control receipt의 원문 packet만 담는다. packet 밖의 DART URL·검색 결과를 원문 근거로 쓰지 않는다. 각 packet metadata의 `rcp_no`를 **정확히 한 번씩** 검토하고, packet path 수는 `source_valid_count`와 같아야 한다. packet이 없는 `source_errors` receipt도 제어 목록에 포함하며 해당 receipt는 `ERROR`/`source_error`로 terminalize한다. 따라서 `source_valid_count + source_error_count`가 제어 수와 같아야 한다.
- broken prehook/control contract(누락 packet, manifest/control 불일치, 중복 receipt, packet 무결성 검증 실패)만 whole-run `scheduler-finish ... error`다. 개별 DART 원문 접근/추출 또는 저장 실패는 해당 receipt를 `ERROR` terminal disposition으로 끝내고, 다음 receipt를 계속 처리한다. `INSUFFICIENT_EVIDENCE`는 source-valid packet을 읽은 뒤 경제적 의미가 부족할 때만 쓴다.
- 전일·당일 두 manifest를 합친 제어 목록의 각 receipt는 정확히 한 terminal disposition을 가져야 한다. `reviewed_rcp_nos`는 source-error가 아닌 receipt와 정확히 일치하고, `source_valid + source_error = control_count`여야 한다. `success_total`(saved/existing/correction_stored/rejected·hold)과 `failure_total`(source_error/store_error/coverage_error)을 category별 합계와 함께 최종 보고에 포함한다. 불일치·누락·중복 terminal audit만 `scheduler-finish ... error`로 종료하며 저장 성공으로 보고하지 않는다.
...
- DART `control_count=0`은 `NO_NEW_DART_FILING`일 뿐 전체 조사 0건이나 `NO_DISCOVERY`가 아니다. DART와 독립적으로 KIND/한국거래소 및 신뢰 가능한 언론 discovery lane은 실제로 실행·완료한 뒤에만 `done`, `count=0`을 쓸 수 있다. 회사 공식 IR·뉴스룸은 optional supplemental lane이므로 시도하지 않아도 되고, 0회 시도·0 valid source가 `done`을 막지 않는다. 시도했다면 각 query/대상·원문 확인 결과 또는 실패 분류를 receipt에 남긴다.
...
- 독립 lane의 검색엔진·WAF·원문 접근 실패는 `NO_DISCOVERY`로 분류하지 않는다. **단일 `web_search` backend 오류로 lane을 즉시 닫지 않는다.** 각 lane은 최대 3회까지 서로 다른 구체 query(회사명+공시명+날짜, `site:` 공식 도메인, 회사명+계약상대방+금액)로 재시도하고, 한 search backend가 오류면 다음 query를 새 호출로 실행한다. 하나라도 URL을 얻으면 `web_extract` 또는 직접 HTTPS 원문으로 확인한다. issuer lane은 DART/KIND의 회사명·홈페이지 단서에서 공식 뉴스룸/IR 도메인을 직접 확인하고, media lane은 복수 신뢰 매체명 query도 시도한다. 모든 DART 및 기타 deterministic direct HTTP 요청(issuer/IR/newsroom을 시도할 때 포함, 재시도 포함)은 기존 shared monotonic request-start pacer로 첫 요청은 즉시, 이후 요청은 0.5~1.0초 간격을 지킨다. `failure_class`는 `redirect_loop|timeout|not_found|extractor_failure|unsupported_or_js` 중 실제 값 하나를 기록하고 다음 URL 조사로 계속한다. 최대 3회 search와 direct-domain 원문 확인이 모두 실패한 뒤에만 `coverage_error`; 각 시도 query·오류 분류·확인 URL을 receipt에 보존한다. 실패 lane과 접근 불가 원문은 terminal audit로 남기고 계속하며, issuer lane의 생략 또는 어떤 supplemental source의 audited failure도 DART control의 `done`을 veto하지 않는다. source-valid 원문을 실제로 확인한 lane만 후보 0건을 `NO_DISCOVERY`로 결론낼 수 있으며, 이 규칙은 발표시각·`known_at`·원문 검증 요구를 완화하지 않는다.

- 회사 홈페이지·IR·뉴스룸·언론·KIND URL을 직접 HTTPS로 열 때는 반드시 `python scripts/giraffe_direct_fetch.py 'https://공식호스트/원문경로'`를 사용한다. 임의 `curl`, `urlopen` 또는 별도 HTTP 클라이언트로 이 경로를 우회하지 않는다. 이 helper와 DART source transport는 동일한 파일 잠금 기반 monotonic request-start authority를 프로세스 간 공유한다. 기본 state는 `~/.hermes/runs/giraffe-http-pacer.json`이며 `GIRAFFE_HTTP_PACER_STATE`를 override하면 모든 프로세스에 같은 경로를 적용한다. state가 없으면 첫 요청은 즉시 시작하고 이후 각 실제 요청·재시도·redirect hop은 최소 0.75초 간격으로 시작한다(응답시간·retry backoff 때문에 더 길어질 수 있다). `web_search`와 `web_extract`는 로컬 pacer의 적용 대상이 아니다. 이 도구들의 호출 간격이 로컬 helper로 보장된다고 보고하지 않는다.
- direct helper는 공개 주소의 HTTPS/443, TLS 검증, 최대 3회 transient 시도·5회 redirect·2 MiB 본문만 허용한다. stdout JSON의 `ok=true`/exit 0이면 추출 본문과 실제 `request_starts`를 검토하며, 이는 발표시각·경제적 의미까지 검증했다는 뜻은 아니다. audit의 `phase=http_request_start`는 DNS 확인·IP 고정·TLS 검증 이후 HTTP GET 전송 직전의 monotonic/UTC 시각이다. 준비 단계에서 실패해 HTTP 전송이 시작되지 않은 시도는 `phase=transport_attempt_start`로 구분한다. `ok=false`/exit 1이면 `failure_class=timeout|redirect_loop|not_found|extractor_failure|unsupported_or_js`를 그대로 URL 실패 audit에 남기고 다음 URL로 계속한다. helper는 오류 원문·헤더·URL query를 출력하지 않는다. `final_url`은 query가 제거된 진단용 경로이므로 query를 가진 원문의 evidence identity로 대신 쓰지 말고 실제 입력 URL과 redirect 결과의 동일성을 별도로 검증한다. 비밀값이 든 URL을 입력하지 않는다. 회사 공식 IR·뉴스룸 direct 수집은 optional이며 생략해도 된다.
...
- 각 DART control candidate는 발표시각 결과와 별개로 `economic_disposition`을 정확히 하나 남긴다: `qualifying_A_or_better|below_threshold|negative_risk|timing_unresolved|error`. 추출한 경제조건, 조건부/확정성, 반대근거, material grade 또는 reject reason을 receipt에 남긴다. 발표시각 미확인은 경제 검토 생략 사유가 아니다: `TIMING_UNRESOLVED`와 경제판정은 병행한다. **공식 원문에서 구속력 있는 공급계약, 계약금액, 전년도 매출 대비 50% 이상 비중, 계약기간을 모두 확인했으면 `qualifying_A_or_better`로 판정하고 `rejected`로 끝내지 않는다.** 선급금 등 지급조건도 확인되면 snapshot에 보존한다. 113개 exact review는 유지하되 material-grade 심층검토는 report class와 DART 경제 사실로 deterministic하게 좁힌 material candidates에 집중한다. 미래 가격 반응은 사용 금지다.
...
- prehook는 API의 durable `giraffe-research-backlog-v1` pending cursor와 terminal audit를 readback한다. terminal DART receipt는 `terminal_exclusions` closed record로 immutable control contract에 남기되 pending control set에는 다시 넣지 않는다. prior `rejected|hold` DART false-negative는 더 이른 terminal record와 exact packet provenance를 `correction_of`에 넣은 새 `-rN` v3 control로만 recover한다. recovery evidence는 새 append-only cursor에 `correction_stored`로 terminalize하고 원 terminal row를 절대 변경하지 않는다. cutoff 뒤에 확인한 correction은 `manual_catch_up`, `eligible_for_original_cutoff=false`, 실제 `known_at`으로 기록하며 scheduled 결과나 08:00/09:05 입력으로 재표시하지 않는다. `carry_forward`는 identity만의 문자열 배열이 아니라 closed record다: DART는 `{identity,kind,payload}`와 `sources`의 exact 동일 packet provenance, discovery는 `{identity,kind,source_url,announcement_at,payload}`를 함께 immutable contract/readback에 보존한다. 동일 DART receipt는 exact 동일 source packet일 때만 union-dedupe하고 packet hash/path/date가 다르면 실패한다. age/lookback으로 pending을 삭제하거나 prior `known_at`을 재사용하지 않는다.
...
- v3 control terminal receipt는 exact `{rcp_no,disposition,evidence_id,economic_disposition,economic_reason,economic_facts}` audit를 남긴다. immutable control의 `sources` 전체와 exact receipt map/list `audits`를 한 번에 넣어 `python -m kr_stock_autotrader.cli dart-terminal-batch '<JSON with sources and audits>'`를 **정확히 한 번** 호출하고, 그 exact ordered `terminal_items`만 completion terminal list의 시작으로 사용한다. batch의 `evidence_requirements`는 terminal item이 아니며, 각 requirement는 event-specific timezone-aware `published_at`으로 evidence-add/readback을 완료한 뒤에만 `saved|correction_stored` item으로 추가한다. 따라서 timestamped qualifying receipt를 `rejected|hold` terminal completion으로 넣지 않는다. 개별 원문/분류 실패는 `disposition=source_error`, 개별 저장/readback 실패는 `disposition=store_error`, 두 경우 모두 `evidence_id=null`, `economic_disposition=error`, 비어 있지 않은 bounded `economic_reason`, `economic_facts=null`로 닫고 다음 receipt를 계속 처리한다. `report_class=other`는 추정으로 닫지 않는다: original source의 HTTPS `source_url`, bounded reason, allowed `{negative_risk,below_threshold,timing_unresolved}` economic disposition과 `rejected|hold` disposition을 가진 explicit non-supply audit를 batch에 제공해야 하며, absent/malformed audit은 fail-closed다. 이로써 buyback, clinical approval, merger 등 positive non-supply filing도 자동 negative/rejected가 아니다. 새 `단일판매ㆍ공급계약체결`은 full-contract facts `{binding_contract,contract_amount,prior_revenue,ratio_percent,term}`를 원문에서 추출하고, 정확히 `[기재정정]`로 시작하는 표준 공급계약 정정 title은 legacy total을 쓰지 말고 closed amendment facts `{binding_contract,economic_basis:"amendment_delta",original_contract_amount,amended_contract_amount,incremental_contract_amount,prior_revenue,incremental_ratio_percent,original_term,amended_term}`를 쓴다. 금액은 bool이 아닌 정수이고 increment는 `amended-original`과 정확히 같아야 하며, 등급은 total이 아닌 양의 increment / prior revenue로 판정한다. qualifying supply contract은 source original에서 event-specific timezone-aware `published_at`을 검증한 경우에만 batch가 `evidence_add_required` requirement를 내며 그 뒤 evidence-add/readback을 한다. `rcept_dt`는 절대 published/announcement time으로 쓰지 않는다. qualifying facts는 있으나 published_at이 없으면 batch의 `hold/timing_unresolved` receipt audit로 닫고 나머지 receipts를 계속 처리한다. 다른 filing class에는 추정 facts를 만들지 말고 `economic_facts=null`로 남긴다. 이는 title-bound closed-shape dispatch이며 v3 schema version은 새 계약 legacy facts와의 호환성을 유지한다.
- prehook terminal lookup은 current manifest exact receipt set만 POST한다(최대 200); unrequested/duplicate/malformed audit row는 fail-closed다. correction은 normal schedule에 없고, one-shot wrapper argument `--rerun-version N --correction-rcp-no <exact-14-digit[,..]>`로만 선택한다. selected receipt는 prior `rejected|hold`, exact source packet provenance, and terminal lookup readback이 모두 일치해야 한다. `.env`에 correction selection을 저장하지 않는다.
- `done`에는 `detail.completion_receipt`로 `schema_version=giraffe-research-completion-v1`, `control_count`, `source_valid`, `reviewed_unique`, `reviewed_rcp_nos`, `source_error`, `store_error`, `coverage_error`, `rejected_after_evidence`, `saved`, `existing`, `correction_stored`, `success_total`, `failure_total`, 그리고 closed `coverage_lanes`를 모두 명시한다. `success_total=rejected_after_evidence+saved+existing+correction_stored`, `failure_total=source_error+store_error+coverage_error`여야 한다. DART terminal semantic counts(coverage_error 제외)의 합은 control_count와 같아야 하고, `coverage_lanes` 후보 수는 DART terminal count와 합산하지 않는다.
- `coverage_lanes`는 정확히 `kind_krx`, `issuer_ir_newsroom`, `reputable_media`만 가진다. 각 lane은 정확히 `{executed:boolean, query_count:int, checked_url_count:int, source_valid_count:int, candidate_count:int, coverage_error_count:int, queries:list[string], checked_sources:list[object]}`다. 실행한 lane의 `queries`는 공백 제거 뒤 비어 있지 않은 실제 검색어이며 1~100개, 각 1~500자, 중복 불가이고 `query_count`와 길이가 정확히 같아야 한다. 생략한 issuer/newsroom lane만 `queries=[]`, `query_count=0`을 허용한다. placeholder·추측·만든 검색어를 쓰지 않는다.
- `checked_sources`의 각 object key는 정확히 `{url,source_valid,published_at,retrieved_at,outcome,failure_class,economic_disposition,economic_reason,evidence_id}`다. `url` literal은 실제로 연 원문 그대로 보존한다. raw ASCII whitespace/control은 금지하며 absolute `https`이고 host가 있고 userinfo/fragment가 없어야 한다. dedupe key는 host IDNA/lowercase 및 trailing dots 제거, default `:443` 제거, empty path와 `/` 동치, path/query의 percent-encoded unreserved decode 및 나머지 percent escape 대문자화로 canonicalize한다; query 순서는 보존한다. canonical URL은 **모든 lane 전체에서** 중복 불가하다. `source_valid`는 strict boolean이다. `published_at`은 `null` 또는 timezone-aware ISO-8601 원문 발표시각 문자열이고, `retrieved_at`은 실제 확인 시각의 timezone-aware ISO-8601 문자열이며 published_at보다 빠를 수 없다. 문자열을 정규화·재작성하지 말고 원문 확인 값을 그대로 보존한다. `outcome`은 정확히 `candidate|negative_evidence|not_material|timing_ineligible|invalid_source` 중 하나다. `candidate`, `negative_evidence`, `not_material`은 `source_valid=true`, non-null `published_at`, `published_at <= cutoff_at_kst`이고 `timing_ineligible`은 같은 조건에서 strict `published_at > cutoff_at_kst`다. `invalid_source`는 `source_valid=false`, `published_at=null`, `failure_class=redirect_loop|timeout|not_found|extractor_failure|unsupported_or_js` 중 하나만 허용한다. source-valid row는 `failure_class=null`이다. candidate는 intermediate `eligible`을 절대 쓰지 않고 terminal `economic_disposition=saved|existing|correction_stored|rejected|hold` 하나와 trim된 1~1000자 `economic_reason`(경제 조건·negative evidence·hold 사유)을 반드시 남긴다. `saved|existing|correction_stored`는 성공한 evidence write/readback 뒤에만 실제 양의 `evidence_id`를 붙인다; 이 ID는 material_evidence에 존재하고 이 source URL과 announcement_at **및 known_at이 모두 cutoff_at_kst 이하**이고 known_at이 announcement_at보다 빠르지 않은 provenance에 일치해야 한다. `rejected|hold`는 inline terminal audit이며 `evidence_id=null`이다. candidate 이외에는 경제 field와 `evidence_id`를 모두 null로 둔다.
- 각 lane은 `checked_url_count=len(checked_sources)`, `source_valid_count=sum(source_valid=true)`, `candidate_count=sum(outcome=candidate)`, `coverage_error_count=sum(source_valid=false)`, `query_count=len(queries)`를 정확히 맞춘다. `kind_krx`와 `reputable_media`는 `executed=true` 및 query 최소 1개가 필요하지만 source-valid 0건이어도 각 URL failure가 위 closed enum으로 audited되면 DONE을 막지 않는다. issuer/newsroom만 optional이며 unattempted면 모든 count와 list가 0/empty여야 한다. completion API가 이 bounded inline receipt를 scheduler DB finish detail에 그대로 영속한다.
- receipt example: `"coverage_lanes":{"kind_krx":{"executed":true,"query_count":1,"checked_url_count":1,"source_valid_count":1,"candidate_count":0,"coverage_error_count":0,"queries":["KIND 2026-09-15 신규 공시"],"checked_sources":[{"url":"https://kind.krx.co.kr/example","source_valid":true,"published_at":"2026-09-15T06:10:00+09:00","retrieved_at":"2026-09-16T06:40:00+09:00","outcome":"not_material","failure_class":null,"economic_disposition":null,"economic_reason":null,"evidence_id":null}]},"issuer_ir_newsroom":{"executed":true,"query_count":1,"checked_url_count":1,"source_valid_count":1,"candidate_count":0,"coverage_error_count":0,"queries":["상장사 IR newsroom 2026-09-15"],"checked_sources":[{"url":"https://issuer.example.com/news","source_valid":true,"published_at":"2026-09-15T05:30:00+09:00","retrieved_at":"2026-09-16T06:42:00+09:00","outcome":"not_material","failure_class":null,"economic_disposition":null,"economic_reason":null,"evidence_id":null}]},"reputable_media":{"executed":true,"query_count":1,"checked_url_count":1,"source_valid_count":1,"candidate_count":0,"coverage_error_count":0,"queries":["국내 상장사 호재 2026-09-15 Reuters"],"checked_sources":[{"url":"https://www.reuters.com/example","source_valid":true,"published_at":"2026-09-15T04:20:00+09:00","retrieved_at":"2026-09-16T06:45:00+09:00","outcome":"negative_evidence","failure_class":null,"economic_disposition":null,"economic_reason":null,"evidence_id":null}]}}`.
...
- terminal disposition, category totals, packet/provenance, 그리고 성공 evidence readback이 모두 정확하면 source_error/store_error 및 audited coverage_error가 있어도 `scheduler-finish ... done`으로 partial completion을 저장한다.
- DART가 빈 manifest(`control_count=0`)여도 KIND와 reputable media lane의 실제 시도 전에는 `done`, count=0이 불가하다. issuer/newsroom은 optional이다. source-valid 후보가 하나라도 있으면 그 후보별 semantic terminal result 없이 done을 호출하지 않는다.
- `STORE_UNVERIFIED` 또는 `STORE_FAILED`는 해당 DART receipt의 `store_error` terminal disposition으로 기록하고 남은 receipt를 계속 처리한다. whole-run error는 broken prehook/control contract에 한정한다.
