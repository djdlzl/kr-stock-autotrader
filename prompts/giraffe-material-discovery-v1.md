...
- v3 control terminal receipt는 exact `{rcp_no,disposition,evidence_id,economic_disposition,economic_reason,economic_facts}` audit를 남긴다. `economic_reason`은 비어 있지 않은 bounded reason이고, 공식 OpenDART 분류가 `dart_single_sale_supply_contract`이면 `economic_facts`는 `{binding_contract,contract_amount,prior_revenue,ratio_percent,term}` 전체를 원문에서 추출한다. binding contract이고 `contract_amount / prior_revenue >= 50%`이면 `rejected|hold`은 API에서 거부되며 evidence-bound `saved|correction_stored`만 가능하다. 다른 filing class에는 추정 facts를 만들지 말고 `economic_facts=null`로 남긴다.
