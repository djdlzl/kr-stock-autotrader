...
- v3 control terminal receipt는 exact `{rcp_no,disposition,evidence_id,economic_disposition,economic_reason,economic_facts}` audit를 남긴다. 각 source마다 먼저 `python -m kr_stock_autotrader.cli dart-terminal-plan '<JSON with source and audit>'`를 호출하고 그 output을 사용한다. 개별 원문/분류 실패는 `disposition=source_error`, 개별 저장/readback 실패는 `disposition=store_error`, 두 경우 모두 `evidence_id=null`, `economic_disposition=error`, 비어 있지 않은 bounded `economic_reason`, `economic_facts=null`로 닫고 다음 receipt를 계속 처리한다. `report_class=other`는 `announcement_at`, `published_at`, 또는 economic facts 없이 helper의 `rejected/negative_risk` terminal audit로 즉시 닫는다. 새 `단일판매ㆍ공급계약체결`은 full-contract facts `{binding_contract,contract_amount,prior_revenue,ratio_percent,term}`를 원문에서 추출하고, 정확히 `[기재정정]`로 시작하는 표준 공급계약 정정 title은 legacy total을 쓰지 말고 closed amendment facts `{binding_contract,economic_basis:"amendment_delta",original_contract_amount,amended_contract_amount,incremental_contract_amount,prior_revenue,incremental_ratio_percent,original_term,amended_term}`를 쓴다. 금액은 bool이 아닌 정수이고 increment는 `amended-original`과 정확히 같아야 하며, 등급은 total이 아닌 양의 increment / prior revenue로 판정한다. qualifying supply contract은 source original에서 event-specific timezone-aware `published_at`을 검증한 경우에만 helper가 `evidence_add_required`를 내며 그 뒤 evidence-add/readback을 한다. `rcept_dt`는 절대 published/announcement time으로 쓰지 않는다. qualifying facts는 있으나 published_at이 없으면 helper의 `hold/timing_unresolved` receipt audit로 닫고 나머지 receipts를 계속 처리한다. 다른 filing class에는 추정 facts를 만들지 말고 `economic_facts=null`로 남긴다. 이는 title-bound closed-shape dispatch이며 v3 schema version은 새 계약 legacy facts와의 호환성을 유지한다.
