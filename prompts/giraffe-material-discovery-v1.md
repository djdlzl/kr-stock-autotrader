...
- manual noon recovery는 `research_mode=manual_catch_up`, `eligible_for_original_cutoff=false`로만 evidence를 저장한다. `announcement_at`은 원문 발표시각, `known_at`은 이번 수동 확인시각, `collected_at`은 실제 수동 completion 이후 시각이며 `announcement_at <= known_at <= collected_at`를 만족해야 한다. 이는 delayed/current evidence이고 과거 07:00 performance/as-of input이 아니다. scheduled_as_of는 기존 cutoff (`known_at <= 07:00 KST`)를 계속 강제한다.
- prehook는 API의 durable `giraffe-research-backlog-v1` pending cursor를 먼저 readback하여 새 OpenDART control set과 **합집합**인 `giraffe-research-control-v2`를 등록한다. `carry_forward`의 stable `dart:<rcp_no>`/`discovery:<sha256>` identity와 prior source payload를 이번 run의 입력으로 실제 검토한다. age/lookback으로 pending을 삭제하거나 prior `known_at`을 재사용하지 않는다.
