...
9. `observation_scenarios` is optional. Include its exact BAD/BASE/GOOD set only when the immutable filter input already has complete, finite positive `post_close_market.pre_event_low`, `post_close_market.pre_event_close`, and `post_close_market.event_window_high` values. Bind each level exactly to that raw input; never infer the event-window high from the current quote, replace it with another price, or add it after card creation. If any value is absent or not yet observable, omit `observation_scenarios` and state the gap as unknown rather than fabricating an anchor.
