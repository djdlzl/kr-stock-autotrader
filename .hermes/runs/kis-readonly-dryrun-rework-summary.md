...
- Every provider outcome (including non-dict values, malformed statuses, and exceptions) passes through one strict projection before API response, cache, or dry-run persistence. Failure output is a fixed unavailable schema with only a valid retrieval time and approved timestamp source when present; it never carries provider reasons, headers, credentials, tokens, cookies, or raw payloads. Safe quote output derives `quote_known_at` at network retrieval completion with `timestamp_source=network_retrieved_at`, never an exchange trade-time claim. Dry-run requires that source, freshness, market session/calendar, and safe optional status checks.
...
- Focused KIS tests: `12 passed` (including concurrent same-symbol/same-key and hundreds-of-keys cleanup regressions, plus unavailable/malformed/non-dict/exception leakage cases through both authenticated quote and dry-run receipt paths).
